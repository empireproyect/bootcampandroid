...
class RemoteDataSourceImpl @Inject constructor(
    ...
    private var mapperProduct: ProductJsonToMapperProduct
): RemoteDataSource {
    ...
    override suspend fun getProducts(): Either<Failure, List<Product>> = withContext(Dispatchers.IO) {
       val response = networkApi.getProducts()
        handlerResponse(
            response,
            data = response.body,
            transform = { it.map { product ->  mapperProduct.mapTo(product)} },
            default = emptyList()
        )
    }
